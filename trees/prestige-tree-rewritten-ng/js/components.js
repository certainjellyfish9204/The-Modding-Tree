...
// data = a function returning the content (actually HTML)
		...
		<div v-if="tmp[layer].upgrades" class="upgTable">
			<div v-for="row in tmp[layer].upgrades.rows" class="upgRow">
				<div v-for="col in tmp[layer].upgrades.cols"><div v-if="tmp[layer].upgrades[row*10+col]!== undefined" class="upgAlign">
					<upgrade :layer = "layer" :data = "row*10+col" :icl = "player[layer].internalNodes!=undefined&&player[layer].internalNodes.includes(row*10+col)?'intBought':(canAffordInternal(layer, row*10+col)?'can':'locked')" :cl = "hasUpgrade(layer, row*10+col)?'bought':(canAffordUpgrade(layer, row*10+col)?'can':'locked')" :pcl="tmp[layer].upgrades[row*10+col].pseudoCan?'can':'locked'" v-bind:style="tmp[layer].componentStyles.upgrade"></upgrade>
				</div></div>
			</div>
			<br>
		</div>
	...
	Vue.component('upgrade', {
		...
		<div v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!==undefined">
			...
			<button v-if="tmp[layer].upgrades[data].internalNode&&player[layer].showInternal" v-on:click="buyInternal(layer,data)" v-bind:class="{ [layer]: true, upg: true, intBought: icl?(icl=='intBought')&&player[layer].internalNodes!=undefined:player[layer].internalNodes.includes(data), locked: (!player[layer].internalNodes.includes(data)&&!canAffordInternal(layer, data)), can: icl?(icl=='can'):(!player[layer].internalNodes.includes(data)&&canAffordInternal(layer, data)), anim: (player.anim&&!player.oldStyle), grad: (player.grad&&!player.oldStyle)}"
				v-bind:style="[((!player[layer].internalNodes.includes(data) && canAffordInternal(layer, data)) ? {'background-color': tmp[layer].color} : {}), tmp[layer].upgrades[data].style]">
				<span v-if= "tmp[layer].upgrades[data].internalTitle"><h3 v-html="tmp[layer].upgrades[data].internalTitle"></h3><br></span>
				<span v-html="tmp[layer].upgrades[data].internalDescription"></span>
