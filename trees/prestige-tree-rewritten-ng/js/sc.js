const SOFTCAPS = {
	...
	spaceBuilding3_2: {
		title: "Tertiary Space Building",
		type: "expRoot",
		start: new Decimal("e1e23"),
		mag: new Decimal(7),
		display() { return player.s.buyables[13].gt(0) && buyableEffect("s", 13).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 7th root" },
	},
	spaceBuilding4: {
	...
	spaceBuilding4_2: {
		title: "Quaternary Space Building",
		type: "expRoot",
		start: new Decimal(`1e23`),
		mag: new Decimal(5),
		display() { return player.s.buyables[14].gt(0) && buyableEffect("s", 14).gte(this.start) },
		info() { return "Starts at ^"+format(this.start)+", exponent brought to the 5th root" },
	},
	spaceBuilding8: {
		title: "Octonary Space Building",
		type: "expRoot",
		start: new Decimal(`1e28`),
		mag: new Decimal(1.75),
		display() { return player.s.buyables[15].gt(0) && buyableEffect("s", 15).gte(this.start) },
		info() { return "Starts at ^"+format(this.start)+", exponent brought to the 1.75th root" },
	},
	spaceBuilding9: {
	...
	spaceBuilding10: {
		...
		type: "expRoot",
		start: new Decimal(190000),
		mag: new Decimal(3),
		display() { return player.s.buyables[20].gt(0) && buyableEffect("s", 20).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the sixth root" },
	},
	s13: {
	...
	hn12_2: {
		title: "Second Honour Upgrade (Honour Boost)",
		type: "expRoot",
		start() { return new Decimal(`ee15`) },
		mag: new Decimal(4),
		display() { return hasUpgrade("hn", 12) && upgradeEffect("hn", 12).gte(this.start()) },
		info() { return "Starts at "+format(this.start())+"x, exponent rooted by 4" },
	},
	hsBuilds: {
	...
	mc13: {
		title: "The Core Fragments Effect",
		type: "root",
		start: new Decimal(2),
		mag: new Decimal(3),
		display() { return buyableEffect('mc',13).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 3th root" },
	},
	mb2: {
		title: "Mastery Building 2 Second Effect",
		type: "root",
		start: new Decimal(2),
		mag: new Decimal(2),
		display() { return buyableEffect('ma',11).eff1.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 2th root" },
	},
	base_db_boost: {
		title: "Node DB effect base",
		type: "log",
		start: new Decimal(1000),
		exp: new Decimal(1.01),
		display() { return player.ai.consc.max(1).pow(0.15).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 3th root" },
	},
		th_2: {
		title: "Thoughts 2nd effect",
		type: "expRoot",
		start: new Decimal(`e30000000`),
		mag: new Decimal(32),
		display() { return tmp.ne.thoughtEff2.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 32th root" },
	},
		th_2_2: {
		title: "Thoughts 2nd effect",
		type: "log",
		start: new Decimal(`e75000000`),
		exp: new Decimal(0.25),
		display() { return tmp.ne.thoughtEff2.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, logarithmical" },
	},
	th_3: {
		title: "Thoughts 3rd effect",
		type: "expRoot",
		start: new Decimal(1e60),
		mag: new Decimal(5),
		display() { return tmp.ne.thoughtEff3.gte(this.start) },
	...
	th_3_2: {
		...
		mag: new Decimal(2),
		display() { return tmp.ne.thoughtEff3.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 50th root" },
	},
	th_1: {
		title: "Thoughts 1st effect",
		type: "expRoot",
		start: new Decimal(250),
		mag: new Decimal(5),
		display() { return tmp.ne.thoughtEff1.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 5th root" },
	},
	mb_2: {
		title: "Mastery Building II",
		type: "expRoot",
		start: new Decimal(15000),
		mag: new Decimal(5),
	...
	g_p: {
		...
		type: "expRoot",
		start: new Decimal(`e1.5e31`),
		mag: new Decimal(1.12),
		display() { return tmp.g.powerEff.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 5th root" },
	},
	ma_s: {
		title: "Free Quirk Imrovements (Space Mastery)",
		type: "expRoot",
		start: new Decimal(`2000`),
		mag: new Decimal(2),
		display() { return tmp.q.impr.decFree.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 2th root" },	
	},
	ma_s_2: {
		title: "Free Quirk Imrovements (Space Mastery)",
		type: "expRoot",
		start: new Decimal(`7500`),
		mag: new Decimal(4),
		display() { return tmp.q.impr.decFree.gte(this.start) },
	...
	cosmic_impr: {
		...
		mag: new Decimal(5),
		display() { return tmp.cs.upgrades[12].effect.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 5th root" },	
	},
	civ3: {
		title: "Civilization 3 effect",
		type: "expRoot",
		start: new Decimal(`ee15`),
		mag: new Decimal(15),
		display() { return Decimal.pow(1e15, tmp.c.power[3]).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 15th root" },	
	},
		civ2: {
		title: "Civilization 2 effect",
		type: "expRoot",
		start: new Decimal(`ee14`),
		mag: new Decimal(15),
		display() { return Decimal.pow(1e20, tmp.c.power[2]).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 15th root" },	
	},
	intnodecb: {
		title: "Internal Node CB",
		type: "log",
		start: new Decimal(`5e7`),
		exp: new Decimal(1),
		display() { return internalNodeEffect('ai',34).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, logarithmic" },	
	},
	rev_eff: {
		title: "Revelations Effect",
		type: "expRoot",
		start: new Decimal(`e5e16`),
		mag: new Decimal(25),
		display() { return tmp.id.revEff.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 25th root" },	
	},
	spectral: {
		title: "Spectal Generators",
		type: "log",
		start: new Decimal(`4e7`),
		exp: new Decimal(1),
		display() { return tmp.sg.spectralTotal.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, logarithmic" },	
	},
	gear_speed: {
		title: "Gear Speed",
		type: "root",
		start: new Decimal(`e4500000`),
		mag: new Decimal(100),
		display() { return tmp.ge.gearSpeed.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 100th root" },	
	},
	asc_p: {
		title: "Ascended Prestige Points effect",
		type: "root",
		start: new Decimal(`1e15`),
		mag: new Decimal(2),
		display() { return tmp.p.ascBoost.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the square root" },	
	},
	asc_p_2: {
		title: "Ascended Prestige Points effect",
		type: "root",
		start: new Decimal(`1e18`),
		mag: new Decimal(4),
		display() { return tmp.p.ascBoost.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the 4th root" },	
	},
	g_p_2: {
		title: "Generator Power Effect",
		type: "expRoot",
		start: new Decimal(`ee45`),
		mag: new Decimal(5),
		display() { return tmp.g.powerEff.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 5th root" },
	},
	time_d: {
		...
		mag: new Decimal(7),
		display() { return buyableEffect('t',12).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 7th root" },
	},
	mile4_cs: {
		title: "Cosmic Powers Milestone 4 effect",
		type: "expRoot",
		start: new Decimal(1e95),
		mag: new Decimal(9),
		display() { return tmp.cs.milestone5Effect.gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, exponent brought to the 9th root" },
	},
	nodecb: {
		title: "Internal Node CB",
		type: "expRoot",
		start: new Decimal(7e8),
		mag: new Decimal(2),
		display() { return internalNodeEffect('ai',34).gte(this.start) },
		info() { return "Starts at "+format(this.start)+"x, brought to the square root" },	
	},
