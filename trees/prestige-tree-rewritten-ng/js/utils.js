...
function canAffordInternal(layer, id, skipAfford=false) {
	let upg = tmp[layer].upgrades[id]
	if (layers[layer].upgrades[id].canAffordIntNode!==undefined && !skipAfford) return upg.canAffordIntNode;
	else if (upg.internalNodeMultiRes) {
		for (let i=0;i<upg.internalNodeMultiRes.length;i++) {
			let cost = upg.internalNodeMultiRes[i].cost
			if (!canAffordPurchase(layer, upg.internalNodeMultiRes[i], cost)) return false;
		}
		return true;
	} else {
		let cost = upg.cost
...
function hasInternal(layer, id){
	return (player[layer].internalNodes.includes(toNumber(id)) || player[layer].internalNodes.includes(id.toString())) && unl(layer)
}
...
function internalNodeEffect(layer, id){
	return (tmp[layer].upgrades[id].internalEffect);
}
...
function buyInternal(layer, id) {
	let upg = tmp[layer].upgrades[id]
	let orig = player;
	if (player[layer].internalNodes.includes(id)) return
	if (!canAffordInternal(layer, id)) return;

	if (upg.internalNodeMultiRes) {
		let doReturn = false;
		for (let i=0;i<upg.internalNodeMultiRes.length;i++) {
			let data = upg.internalNodeMultiRes[i]
			let cost = data.cost;
			if (data.currencyInternalName){
				let name = data.currencyInternalName
				if (data.currencyLocation){
	...
	player[layer].internalNodes.push(id);
	if (upg.onPurchase != undefined)
		upg.onPurchase()
}
